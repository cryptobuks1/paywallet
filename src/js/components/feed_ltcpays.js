    ...
    failoverAPI("get_order_matches", {'filters': filters, 'filterop': 'or', status: 'pending'},
      function(data, endpoint) {
        ...
        if(AUTO_LTC_ESCROW_ENABLE) {
          ...
          //Check if any of these order matches are being covered by the autoltcescrow server. if so, skip them
...
function UpcomingLTCPayFeedViewModel() {
  ...
  self.process = function(ltcPayData) {
    ...
    if(PREFERENCES['ltcpay_method'] == 'auto') {
    ...
    } else if (PREFERENCES['ltcpay_method'] == 'manual') {
